...
pub async fn ingest_directory(
    ...
    // Prove the root is a readable directory before anything else. A missing or
    // mistyped path used to walk nothing -- the walker's errors were skipped --
    // and report success, having removed every ingested row in the namespace.
    std::fs::read_dir(dir_path).map_err(|e| {
        anyhow::anyhow!(
            "Refusing to ingest {:?} into namespace {}: it is not a readable directory, so nothing was changed: {}",
            dir_path,
            namespace,
            e
        )
    ...
    //
    // A failure here fails the ingest rather than warning about it. Rewriting
    // the nodes already took their GOVERNS edges, so reporting success would
    // hand back a graph whose rules reach none of their code.
